from database.models.base import User, ContestAccess, Task, Contest, Solution, SolutionStatusEnum
...
async def task_solution(  # contest/(slug:str)/task/(slug:str)/solution
    ...
    access: ContestAccess | None = await get_access_by_user_and_contest(db,user.id,contest.id)
