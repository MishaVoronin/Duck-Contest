...
async def get_list_of_last_news(db: AsyncSession, last: str | None):
    if last is None:
        ...
        news: list[News] = news_crud.get_the_last_news_after_a_certain_time(
            db, last_news.created_at, 5
        )

    return news_schemas.ListOfNewsInfoResponse(
        news=[
            news_schemas.NewsInfoResponse(
                name=i_news.name,
                slug=i_news.slug,
                text=i_news.text,
            )
...
async def create_news(
    db: AsyncSession, user: User, data: news_schemas.CreateNewsInput
) -> None:
    if user is None:
        raise HTTPException(
